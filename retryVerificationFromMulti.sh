...
echo "$deployments" | while IFS= read -r deployment; do
  transactions=$(echo "$deployment" | jq -c '.transactions[] | select(.transactionType == "CREATE" or .transactionType == "CREATE2") | {arguments: (.arguments // []), contractName: .contractName, contractAddress: .contractAddress}')
  echo "Number of 'CREATE' or 'CREATE2' transactions: $(echo "$transactions" | jq -s 'length')"

  chainId=$(echo "$deployment" | jq -r '.chain')
  echo "Chain ID: $chainId"

  if [ -z "$transactions" ]; then
    echo "No 'CREATE' or 'CREATE2' transactions found in the current deployment."
    continue
  fi
